...
The runner keeps solver budgets fixed, executes points sequentially, saves every
run, and writes CSV/JSON/Markdown/LaTeX/PNG evidence plus a guarded reviewer
response. A fast profile is available for development, but only the evidence
profile can support the bounded statement that IEEE-118 executed successfully.
"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "Repeat once per grid case, for example: "
            "--case IEEE24_v2=config/wp3_optim/IEEE24_scheduler_v2.json "
            "--case IEEE118_v3=config/wp3_optim/IEEE118_v3.json."
        ),
    )
...
def _points(
    ...
    outage_values = sorted(set(outages or _auto_scales(int(dimensions["outages"]))))
    horizon_values = sorted(set(horizons or _auto_scales(int(dimensions["horizon"]), int(dimensions["max_duration"]))))
    points = [BenchmarkPoint("grid", label, config, int(case_dimensions[label]["buses"])) for label, config in cases]
    points += [BenchmarkPoint("outages", reference, reference_config, value, outage_count=value) for value in outage_values]
    points += [BenchmarkPoint("horizon", reference, reference_config, value, horizon_steps=value) for value in horizon_values]
    return points
...
def _setting_overrides(args: argparse.Namespace, section: str) -> dict[str, Any]:
    ...
    if section == "clustered_cvar":
        values["scenario_count"] = args.scenario_count
...
def main_scalability(argv: Sequence[str] | None = None) -> int:
    ...
    if args.grid_case is None:
        args.grid_case = [
            _labelled_path(
                "IEEE24=config/wp3_optim/IEEE24_scheduler.json"
            )
        ]
    ...
    dimensions = case_dimensions[reference]
    plan = _points(cases, reference, case_dimensions, args.outage_counts, args.horizons)
...
if __name__ == "__main__":
    raise SystemExit(main_scalability())
