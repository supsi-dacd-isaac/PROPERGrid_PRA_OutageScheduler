...
def add_variables(SCOS_model, T, outages, generators_names, n_minus1_names, nodes_names, branch_names):
    ...
    start_t = SCOS_model.addVars(T, outages['names'], vtype=GRB.BINARY, name="xt_lines_and_gens")
    end_t = SCOS_model.addVars(T, outages['names'], vtype=GRB.BINARY, name="xt_lines_and_gens")
    ...
    d_worst_case_N_minus_c = SCOS_model.addVar(lb=0, name="worst-case-curtailment-N-contingency")
    d_curt_tb = SCOS_model.addVars(T, nodes_names, lb=0, name="d_curt_tb")
    d_curt_tbc = SCOS_model.addVars(T, nodes_names, n_minus1_names, lb=0, name="d_curt_tbc")

    # Power flow variables
    flow_tl = SCOS_model.addVars(T, branch_names, lb=-1e4, ub= 1e4, name="flow_tl")
    flow_tlc = SCOS_model.addVars(T, branch_names, n_minus1_names, lb=-1e4, ub= 1e4, name="flow_tlc")

    # Phase angle variables (bounded between -0.3 and 0.3 radians)
    theta_tb = SCOS_model.addVars(T, nodes_names, lb=-360, ub=360, name="theta_tb")
    theta_tbc = SCOS_model.addVars(T, nodes_names, n_minus1_names, lb=-360, ub=360, name="theta_tbc")
    return SCOS_model, xt, p_t_gen, p_tn1c_gen, d_worst_case, d_worst_case_N_minus_c, d_curt_tb, d_curt_tbc, flow_tl, flow_tlc, theta_tb, theta_tbc, start_t, end_t


def add_planned_outages_constraints(SCOS_model, T, outages_names,
                                    xt, start_t, end_t,
                                    max_simulataneous_tasks_t,
                                    dic_outage_duration, n_planning_steps):
                                    
    # Duration constraints
    for o in outages_names:
    ...
    for o in outages_names:
        for t in range(1, n_planning_steps):
            SCOS_model.addConstr(start_t[T[t], o] <= xt[T[t], o], name=f"Start_con_{o}_{t}")
            SCOS_model.addConstr(start_t[T[t], o] >= xt[T[t], o] - xt[T[t-1], o],
                                 name=f"start_outage_con_{o}_{t}")
            SCOS_model.addConstr(end_t[T[t], o] <= xt[T[t], o], name=f"Start_con_{o}_{t}")
            SCOS_model.addConstr(end_t[T[t-1], o] >= xt[T[t-1], o] - xt[T[t], o],
                                 name=f"end_outage_con_{o}_{t}")
    return SCOS_model
        ...
        for g in generators_names:
            if g in outages_names:
                SCOS_model.addConstr(p_t_gen[t, g] <= Pg_max[g] * (1 - xt[t, g]),
                                     f"gen_max{t}_{g}_with_scheduled_outage")
                # SCOS_model.addConstr(p_t_gen[t, g] >= Pg_min[g] * (1 - xt[t, g]), f"gen_min_{t}_{g}_with_scheduled_outage")
            else:
                SCOS_model.addConstr(p_t_gen[t, g] <= Pg_max[g], f"gen_max{t}_{g}")
                # SCOS_model.addConstr(p_t_gen[t, g] >= Pg_min[g], f"gen_min_{t}_{g}")
            SCOS_model.addConstr(p_t_gen[t, g] >= 0, f"gen_min_{t}_{g}")
            for c in n_minus1_names:
                if c[3:] == g:
                    SCOS_model.addConstr(p_tn1c_gen[t, g, c] == 0,
                                         f"gen_max_{t}_{g} = N_minus_1_{c} (null gen)")
                elif g in outages_names:
                    SCOS_model.addConstr(p_tn1c_gen[t, g, c] <= Pg_max[g] * (1 - xt[t, g]),
                                         f"gen_max_{t}_{g}_N_minus_1_{c}")
                    #SCOS_model.addConstr(p_tn1c_gen[t, g, c] >= Pg_min[g] * (1 - xt[t, g]), f"gen_min_{t}_{g}_N_minus_1_{c}")
                else:
                    SCOS_model.addConstr(p_tn1c_gen[t, g, c] <= Pg_max[g],
                                         f"gen_max_{t}_{g}_N_minus_1_{c}")
                    # SCOS_model.addConstr(p_tn1c_gen[t, g, c] >= Pg_min[g],  f"gen_min_{t}_{g}_N_minus_1_{c}")
                SCOS_model.addConstr(p_tn1c_gen[t, g, c] >= 0, f"gen_min_{t}_{g}_N_minus_1_{c}")
    return SCOS_model
...
def add_nodal_power_balance_constraints(SCOS_model, T, nodes_names, branch_names, generators_names, S_lk, nodal_demand,
                                        d_curt_tb, d_curt_tbc, n_minus1_names,
                                        p_t_gen, p_tn1c_gen, flow_tl, flow_tlc, gen_is_in_bus, d_worst_case, d_worst_case_N_minus_c):
    for t_idx, t in tqdm(enumerate(T), desc="Adding Constraints", total=len(T), ncols=100, colour="green"):
        ...
        for b_idx, b in enumerate(nodes_names):  # Add Nodal Power Balance total_Flow = Production - Demand  constraint using DC Power Flow approximation
            demand_tb = nodal_demand.iat[t_idx, b_idx]
            power_injection_tb = quicksum([S_lk.at[l, b] * flow_tl[t, l] for l in branch_names])
            if b in gen_is_in_bus:  # do we have generators in node b?
                g = [generators_names[idx] for idx, g2b in enumerate(gen_is_in_bus) if g2b == b][0]
                SCOS_model.addConstr(demand_tb - power_injection_tb - p_t_gen[t, g] <= d_curt_tb[t, b], f"Power_Balance_{t}_{b}")
            else:  # no gen?
                SCOS_model.addConstr(demand_tb - power_injection_tb <= d_curt_tb[t, b], f"Power_Balance_{t}_{b}")
            for c in n_minus1_names:  # Add Nodal Power Balance for unplanned failures ..... N-1 contingency cases
                power_injection_tbc = quicksum([S_lk.at[l, b] * flow_tlc[t, l, c] for l in branch_names])
                if b in gen_is_in_bus:
                    g = [generators_names[idx] for idx, g2b in enumerate(gen_is_in_bus) if g2b == b][0]
                    tmp_c = demand_tb - power_injection_tbc - p_tn1c_gen[t, g, c] <= d_curt_tbc[t, b, c]
                    SCOS_model.addConstr(tmp_c, f"Power_Balance_{t}_{b}_{c}")
                else:
                    tmp_c = demand_tb - power_injection_tbc <= d_curt_tbc[t, b, c]
                    SCOS_model.addConstr(tmp_c, f"Power_Balance_{t}_{b}_{c}_nogen")
    # add mean curtailment constraint
    """mean_demand_curtailed_n1 = quicksum([d_curt_tbc[t, b, c] for t in T for b in nodes_names for c in n_minus1_names])
    mean_demand_curtailed_normal = quicksum([d_curt_tb[t, b] for t in T for b in nodes_names])

    SCOS_model.addConstr(mean_demand_curtailed_n1 <= d_worst_case, f"Max_Demand_Curtailment_{t}_{b}_{c}")
    SCOS_model.addConstr(mean_demand_curtailed_normal <= d_worst_case, f"Max_Demand_Curtailment_{t}_{b}")
    """
    # For N-1 scenario
    max_demand_curtailed_n1 = SCOS_model.addVar(name="max_demand_curtailed_n1")  # Maximum for N-1 scenario
    max_demand_curtailed_normal = SCOS_model.addVar(name="max_demand_curtailed_normal")  # Maximum for normal scenario
    SCOS_model.addGenConstrMax(max_demand_curtailed_n1,
                               [d_curt_tbc[t, b, c] for t in T for b in nodes_names for c in n_minus1_names],
                               name="Max_Demand_Curtailment_N1")
    SCOS_model.addGenConstrMax(max_demand_curtailed_normal,
                               [d_curt_tb[t, b] for t in T for b in nodes_names],
                               name="Max_Demand_Curtailment_Normal")
    SCOS_model.addConstr(max_demand_curtailed_n1 <= d_worst_case_N_minus_c, name="Max_Demand_Curtailment_Limit_N1")
    SCOS_model.addConstr(max_demand_curtailed_normal <= d_worst_case, name="Max_Demand_Curtailment_Limit_Normal")
    return SCOS_model
        ...
        for l_idx, l in enumerate(branch_names):
            ...
            for c in n_minus1_names:
                if not(c[3:] == l) and l in outage_names:
                    dc_pf_contingency = (flow_tlc[t, l, c] == branch_susceptance[l] * (theta_tbc[t, from_b, c] - theta_tbc[t, to_b, c]))
                    SCOS_model.addConstr(dc_pf_contingency, f"DCPF_{t}_{l}_{c}")

    return SCOS_model


def preprecess_data_guroby(data):
    # Preprocess the data
    net = data['network']
    outages = data['outages']
    nodal_demand = data['nodal_demand']
    ...
    S_lk = pd.DataFrame(Incidence, index=branch_names, columns=nodes_names)
    ...
    return (T, outages, nodal_demand, n_minus1_names, reference_bus, num_buses, num_branches,
...
def optimization_SCOS_model_gurobi(data):
    ...
    try:
        ...
        (SCOS_model, xt, p_t_gen, p_tn1c_gen, d_worst_case, d_worst_case_N_minus_c, d_curt_tb,
         d_curt_tbc, flow_tl, flow_tlc, theta_tb, theta_tbc, start_t, end_t) = add_variables(
        ...
        Objective_fun -= (0.5 * d_worst_case + 0.05 * d_worst_case_N_minus_c)  # Add the curtailment term
        ...
        logger.info(
            ...
            f' - d_worst_case_N_minus_c: Worst-case demand curtailment under N-1 cases\n'
            f' - d_curt_tb: Curtailment undamaged (scheduled damage/outage) for all t and all nodes b\n'
            ...
            f'{blue_c}Objective Function Defined: \sum x_t_o * priority_o - d_curtailed_worst_case   {reset_c}\n'
        )

        logger.info('adding constraints for scheduled outages: duration, number of tasks, continuity')
        SCOS_model = add_planned_outages_constraints(SCOS_model, T, outages_names,  xt, start_t, end_t, max_tasks_t, dic_outage_duration, n_planning_steps)
        ...
        SCOS_model = add_nodal_power_balance_constraints(SCOS_model, T, nodes_names, branch_names, generators_names,
                                                         S_lk, nodal_demand, d_curt_tb, d_curt_tbc, n_minus1_names,
                                                         p_t_gen, p_tn1c_gen, flow_tl, flow_tlc, gen_is_in_bus,
                                                         d_worst_case, d_worst_case_N_minus_c)
        ...
        if SCOS_model.status == GRB.OPTIMAL:
            logger.info(f"{red_c} Optimal solution found!{reset_c}{blue_c}:-) :-):-){reset_c}")
            # Retrieve and save variable values
            solution = {v.VarName: v.X for v in SCOS_model.getVars()}
            # Save the solution to a file or database
            with open("optimal_solution.json", "w") as f:
                json.dump(solution, f)

        elif SCOS_model.status == GRB.INFEASIBLE:
            logger.warning(f"{red_c} Model is infeasible! :-(:-(:-({reset_c}")

            # Run IIS to find conflicting constraints
            SCOS_model.computeIIS()
            SCOS_model.write("model.ilp")  # Write IIS to a file for inspection
            print("Conflicting constraints are:")
            for c in SCOS_model.getConstrs():
                if c.IISConstr:
                    print(c.ConstrName)

        elif SCOS_model.status == GRB.UNBOUNDED:
            ...
            logger.info(f"Optimization was stopped with status: {SCOS_model.status}")
