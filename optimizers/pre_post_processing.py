...
from pra_psa.utils.data_preporcess import aggregate_hourly_demand, aggregate_step_costs_and_durations
...
def load_data_from_conf_grid_case(conf_path=None):
    ...
    if conf_path is None:
        ...
        possible_paths = [
            os.path.join(project_root, 'config', 'conf_IEEE24.json'),
            os.path.join(os.path.dirname(project_root), 'config', 'conf_IEEE24.json'),
            os.path.join(os.path.dirname(os.path.dirname(project_root)), 'config', 'conf_IEEE24.json'),
            'C:\\Users\\roberto.rocchetta.in\\OneDrive - SUPSI\\Documenti\\GitHub\\PROPER\\config\\conf_IEEE24.json']
...
def post_process_results_older_version(res_path_name, names, T):
    """ loading gurobi solution and post-processing the results"""
    try:
...
def post_process_results(solution, names, T):
    """Load Gurobi solution and post‑process results into DataFrames."""
